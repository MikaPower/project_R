    ...
    for (excel in excel_files) {
      ...
      while (row <= nrow(data)) {
        ...
        if (!is.na(axxis) && abs(axxis) > limit_val) {
          ...
          full_df <- rbind(full_df,small_matrix)
        }
        else{
...
full_df <- cbind(full_df,labels_matrix)
write.csv(full_df, paste0(directory_to_save, '/', 'final_df_accX_only', ".csv"), row.names = FALSE)
